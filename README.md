...
<body>
    ...
    <script>
        // 将所有全局函数提前定义并挂载到 window 对象上，防止 ReferenceError
        ...
        async function updateSubmissionStatus(name, submitted) {
            await supabaseClient.from('assessments').update({ submitted }).eq('student_name', name);
        }
        ...
        async function renderClass() {
            ...
            if (!classData) {
                console.error(`渲染班级失败：找不到ID为 "${classId}" 的班级`);
                document.getElementById('mainContainer').innerHTML = `<div style="max-width:700px;margin:40px auto;text-align:center;"><h2>错误：找不到班级</h2><p>当前班级ID: ${classId}</p></div>`;
                return;
            }
            document.getElementById('mainContainer').innerHTML = `
            <div style="max-width:700px;margin:0 auto;padding:20px;">
                <h2>${role==='student'?'📋 选择名字':'📋 '+classData.name}</h2>
                <p style="margin-bottom:20px;">${classData.name} · ${classData.students.length}人</p>
                <div class="student-grid">
                    ${classData.students.map(n=>{
        ...
        window.exportPDF = name => {
            ...
            const className = CLASS_DATA.find(c => c.id === classId)?.name || '未知班级';
            let html = `<html><head><meta charset="UTF-8"><style>body{font-family:SimSun,sans-serif;padding:20px;}h1{text-align:center;}h2{background:#eef;padding:6px;}.item{margin:10px 0;border-left:3px solid #333;padding-left:12px;}img{max-width:100%;max-height:400px;}</style></head><body><h1>${name} 品德评定报告</h1><p>班级：${className}</p><hr>`;
